...
import { SketchSettings, tetrahedron } from "./tetrahedron-geometry";
...
export default class Tetrahedron {
  ...
  draw = () => {
    ...
    group.addChild(tetrahedron(center, paper.view.size, radius, this.settings));
  };

  saveImage = () => {
    saveImage(this.canvas, "tetrahedron");
  };

  saveSVG = () => {
    saveSVG(paper.project, "tetrahedron");
  };
}
...
export class GUIIcosahedron extends GUIController {
  ...
  constructor(
    gui: FolderApi,
    public target: Tetrahedron,
  ) {
    super(gui);
    this.gui = this.addFolder(gui, { title: "Tetrahedron" });
