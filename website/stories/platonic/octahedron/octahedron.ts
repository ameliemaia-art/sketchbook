...
import { octahedron, SketchSettings } from "./octahedron-geometry";
...
export default class Octahedron {
  ...
  draw = () => {
    ...
    group.addChild(octahedron(center, paper.view.size, radius, this.settings));
  };

  saveImage = () => {
    saveImage(this.canvas, "octahedron");
  };

  saveSVG = () => {
    saveSVG(paper.project, "octahedron");
  };
}

export class GUIOctahedron extends GUIController {
  gui: FolderApi;

  constructor(
    gui: FolderApi,
    public target: Octahedron,
  ) {
    super(gui);
    this.gui = this.addFolder(gui, { title: "Octahedron" });
