...
class ExtrudeGeometry extends BufferGeometry {
   ...
   * @param {ExtrudeGeometryOptions} [options] - The extrude settings.
   */
  constructor(
    ...
    function addShape(shape) {
      ...
      function mergeOverlappingPoints(points) {
        ...
        for (let i = 1; i <= points.length; i++) {
          ...
          const thresholdSqScaled =
            THRESHOLD_SQ * scalingFactorSqrt * scalingFactorSqrt;
      ...
      for (let i = 0; i < vlen; i++) {
        ...
        if (!extrudeByPath) {
          // Apply taper scaling at the start (z=0)
          const scaleFactor = taperStart;
          v(vert.x * scaleFactor, vert.y * scaleFactor, 0);
        } else {
          // v( vert.x, vert.y + extrudePts[ 0 ].y, extrudePts[ 0 ].x );

          normal!
            .copy(splineTube!.normals[0])
            .multiplyScalar(vert.x * taperStart);
          binormal!
            .copy(splineTube!.binormals[0])
            .multiplyScalar(vert.y * taperStart);
        ...
        for (let i = 0; i < vlen; i++) {
          ...
          if (!extrudeByPath) {
            ...
            normal!
              ...
              .multiplyScalar(vert.x * scaleFactor);
            binormal!
              .copy(splineTube!.binormals[s])
              .multiplyScalar(vert.y * scaleFactor);
      ...
      for (let b = bevelSegments - 1; b >= 0; b--) {
        ...
        for (let i = 0, il = contour.length; i < il; i++) {
          const vert = scalePt2(contour[i], contourMovements[i], bs);
          // Apply taper scaling at the end (front face)
          v(vert.x * taperEnd, vert.y * taperEnd, depth + z);
        }
